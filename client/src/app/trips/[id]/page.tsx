...
export default function TripDetailPage() {
  ...
  const handleDaySelect = async (dayId: number) => {
    try {
      ...
      setError(err.response?.data?.error || "Failed to load day details");
    }
  };
  ...
  const handleCreateSelection = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || "Failed to create selection");
    }
  };
  ...
  const handleDeleteSelection = async (selectionId: number) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || "Failed to delete selection");
    }
  };
